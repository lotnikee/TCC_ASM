...
def O2_ads():
    ...
    O2_ads_denominator = 2 * vib_O2 * rot_O2 * trans_O2 * np.sqrt(2 * np.pi * m_O2 * boltzmann_J * temperature)
...
def O2_des():
    ...
    O2_des_denominator = h_J * (vib_O_surface ** 2)
    return O2_des_numerator / O2_des_denominator

### Define a function to determine the CO oxidation reaction rate
def CO_oxi():
    CO_oxi_numerator = boltzmann_J * temperature * vib_CO_oxidation * np.exp((-CO_transition_energy) / (boltzmann_ev * temperature))
    CO_oxi_denominator = h_J * vib_CO_surface * vib_O_surface
...
def CO_oxi_rev():
    ...
    CO_oxi_rev_denominator = 2 * vib_CO2 * rot_CO2 * trans_CO2 * np.sqrt(2 * np.pi * m_CO2 * boltzmann_J * temperature)
